...
public class RPGEnchantingClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		BlockEntityRenderers.register(EntityRegistry.RPG_ENCHANTING_TABLE, RPGEnchantingTableBlockEntityRenderer::new);
		MenuScreens.register(ScreenHandlerTypesRegistry.RPG_ENCHANTMENT_SCREEN_HANDLER, RPGEnchantmentScreen::new);
		initializeClientEvents();
	}

	public static void initializeClientEvents() {
		ItemTooltipCallback.EVENT.register((stack, context, type, lines) -> {
			ResolvableProfile playerEnchantedComponent = stack.get(RPGEnchanting.PLAYER_ENCHANTED);
			if (playerEnchantedComponent != null && CLIENT_CONFIG.show_item_tooltip_enchanted_by_player_name.get()) {
				...
				lines.add(Component.translatable("item.additional_tooltip.player_relation.enchanted_by", formatting_string + playerEnchantedComponent.partialProfile().name()));
			}
		});
	}
}