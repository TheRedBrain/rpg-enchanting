...
public class RPGEnchantingTableBlockEntityRenderer implements BlockEntityRenderer<RPGEnchantingTableBlockEntity> {
	...
	public void render(
			RPGEnchantingTableBlockEntity rpgEnchantingTableBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j
	) {
		matrixStack.push();
		matrixStack.translate(0.5F, 0.75F, 0.5F);
		float g = (float) rpgEnchantingTableBlockEntity.ticks + f;
		matrixStack.translate(0.0F, 0.1F + MathHelper.sin(g * 0.1F) * 0.01F, 0.0F);
		float h = rpgEnchantingTableBlockEntity.bookRotation - rpgEnchantingTableBlockEntity.lastBookRotation;
		...
		float k = rpgEnchantingTableBlockEntity.lastBookRotation + h * f;
		...
		float l = MathHelper.lerp(f, rpgEnchantingTableBlockEntity.pageAngle, rpgEnchantingTableBlockEntity.nextPageAngle);
		float m = MathHelper.fractionalPart(l + 0.25F) * 1.6F - 0.3F;
		float n = MathHelper.fractionalPart(l + 0.75F) * 1.6F - 0.3F;
		float o = MathHelper.lerp(f, rpgEnchantingTableBlockEntity.pageTurningSpeed, rpgEnchantingTableBlockEntity.nextPageTurningSpeed);
